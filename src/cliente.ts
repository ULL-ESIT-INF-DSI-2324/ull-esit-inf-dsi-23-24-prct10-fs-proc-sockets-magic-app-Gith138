...
import { Cartas, Color, Tipo, Rareza, MostrarCartas } from './magic-app.js';
import net from 'net';
...
argv = yargs(hideBin(process.argv))
  .command(
    ...
    (argv) => {
      ...
      const info = JSON.stringify({ accion: 'actualizar',  carta: cartas, usuario: argv.usuario, fin: 'FIN' });
      cliente.write(info);
    },
  )
.help().argv;
...
cliente.on('end', () => {
  ...
  switch (respuesta.tipo) {
    ...
    case 'SuccessCarta':
      receivedData = JSON.parse(respuesta.respuesta);
      console.log(`--------------------------------`);
      MostrarCartas(receivedData.toString());
      console.log(`--------------------------------`);
      break;
  }
});
